...
namespace sca {
    ...
    public:
         ...
         */
#ifdef _RH_SHARED_BUFFER_DEBUG
        typedef ::sca::debug::checked_iterator<const value_type*,shared_buffer> iterator;
#else
...
#ifdef _RH_SHARED_BUFFER_DEBUG
        typedef ::sca::debug::checked_iterator<value_type*,buffer> iterator;
#else
        typedef value_type* iterator;
#endif
        /// @brief  A random access iterator to const value_type.
#ifdef _RH_SHARED_BUFFER_DEBUG
        typedef ::sca::debug::checked_iterator<const value_type*,buffer> const_iterator;
#else
        typedef const value_type* const_iterator;
#endif
        /// @brief  The default allocator class.
#ifdef _RH_SHARED_BUFFER_DEBUG
        typedef ::sca::debug::checked_allocator<T> default_allocator;
#elif defined(_RH_SHARED_BUFFER_USE_STD_ALLOC)
        typedef std::allocator<T> default_allocator;
#else
        typedef ::sca::shm::HybridAllocator<T> default_allocator;
#endif
     ...
     */
    template <class T>
    inline sca::buffer<T> make_buffer(T* data, size_t size)
    {
        return sca::buffer<T>(data, size);
    }
    ...
    inline sca::buffer<T> make_buffer(T* data, size_t size, D deleter)
    {
        return sca::buffer<T>(data, size, deleter);
    }
