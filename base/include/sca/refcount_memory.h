...
namespace sca {
    ...
    namespace detail {
        ...
        template <typename T>
        struct is_process_shared< ::sca::shm::Allocator<T> > {
            static const bool value = true;
        };

        // Traits class specialization for REDHAWK's hybrid memory allocator
        template <typename T>
        struct is_process_shared< ::sca::shm::HybridAllocator<T> > {
            static const bool value = true;
        };
    }
     ...
     * structure like sca::shared_buffer to manage the reference counting
