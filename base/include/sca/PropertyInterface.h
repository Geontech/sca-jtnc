...
#include "CF/CF.h"
#include "CF/CFProperties.h"
//#include "ossie/Port_impl.h"

#include "sca/ComplexProperties.h"
#include "sca/equals.h"
...
protected:
    ...
    // change listener registration for internal notification support classes
    sca::notification<void (void)>                            voidListeners_;
...
protected:
    ...
    bool equals (const value_type* oldValue, const value_type* newValue)
    {
        if (oldValue && newValue) {
            ...
            return sca::internal::equals(*oldValue, *newValue);
        } else {
    ...
    {
        throw sca::not_implemented_error("allocate");
    }
    ...
    {
        throw sca::not_implemented_error("deallocate");
    }
...
private:
    ...
    typedef sca::callback<value_type()> QueryFunc;
    typedef sca::callback<void (const value_type&)> ConfigureFunc;
    typedef sca::callback<bool (const value_type&)> AllocateFunc;
    typedef sca::callback<void (const value_type&)> DeallocateFunc;
    ...
    sca::notification<void (const value_type*, const value_type*)> pointerListeners_;
    sca::notification<void (const value_type&, const value_type&)> valueListeners_;
};
