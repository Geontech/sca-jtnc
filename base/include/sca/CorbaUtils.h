...
namespace sca {
	...
	std::vector<std::string> listContext(const CosNaming::NamingContext_ptr ctx, const std::string &dname );
        ...
        struct OrbContext {
          ...
          void init(){
            orb = sca::corba::Orb();
            rootPOA = sca::corba::RootPOA();
            namingService = sca::corba::InitialNamingContext();
            ...
            if (_a >>= seq) {
                ...
                if (length == 0) {
                    ...
                    for (unsigned int idx=0; idx<seq->length(); idx++) {
                        _s[idx] = sca::corba::returnString((*seq)[idx]);
                    }
                }
                return true;
            }
...
{                                                                   \
    return sca::corba::vector_extract<T,SEQ>(_a, _s);             \
}                                                                   \
inline void operator <<= (CORBA::Any& _a, const std::vector<T>& _s) \
{                                                                   \
    sca::corba::vector_insert<T,SEQ,T>(_a, _s);                   \
}                                                                   \
...
inline bool operator >>= (const CORBA::Any& _a, std::vector<T>& _s) \
{                                                                   \
    return sca::corba::vector_extract_convert<T,SEQ>(_a, _s);     \
}                                                                   \
inline void operator <<= (CORBA::Any& _a, const std::vector<T>& _s) \
{                                                                   \
    sca::corba::vector_insert_convert<T,SEQ>(_a, _s);             \
}
