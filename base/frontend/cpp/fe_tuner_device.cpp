 ...
 *
 * SCA-JTNC frontendInterfaces is free software: you can redistribute it and/or modify it
...
namespace frontend {
    PREPARE_LOGGING(FrontendTunerDevice);
    ...
    void FrontendScanningTunerDevice<TunerStatusStructType>::checkValidIds(const CF::Properties & capacities) {
        ...
        for (unsigned int ii = 0; ii < capacities.length(); ++ii) {
            ...
            if (id != "FRONTEND::tuner_allocation" && id != "FRONTEND::listener_allocation" && id != "FRONTEND::scanner_allocation"){
                throw CF::CapacityManagement::InvalidCapacity("UNKNOWN ALLOCATION PROPERTY1", capacities);
            }
            ...
            if(!property){
                throw CF::CapacityManagement::InvalidCapacity("UNKNOWN PROPERTY", capacities);
            }
            try{
                property->setValue(capacities[ii].value);
            }
            catch(const std::logic_error &e){
                throw CF::CapacityManagement::InvalidCapacity("COULD NOT PARSE CAPACITY", capacities);
            };
        }
    }
    ...
    *******************************************************************************************/
    ...
    /*****************************************************************/
    template < typename TunerStatusStructType >
    CF::CapacityManagement::UsageType FrontendTunerDevice<TunerStatusStructType>::updateUsageState() {
        ...
        if (tunerAllocated == tuner_allocation_ids.size())
            return CF::CapacityManagement::BUSY;
        // Else, device is active
        return CF::CapacityManagement::ACTIVE;
    }
    ...
    void FrontendTunerDevice<TunerStatusStructType>::checkValidIds(const CF::Properties & capacities) {
        for (unsigned int ii = 0; ii < capacities.length(); ++ii) {
            const std::string id = (const char*) capacities[ii].id;
            if (id == "FRONTEND::scanner_allocation"){
                throw CF::CapacityManagement::InvalidCapacity("FRONTEND::scanner_allocation found in allocation; this is not a scanning device", capacities);
            }
        }
        for (unsigned int ii = 0; ii < capacities.length(); ++ii) {
            const std::string id = (const char*) capacities[ii].id;
            if (id != "FRONTEND::tuner_allocation" && id != "FRONTEND::listener_allocation"){
                throw CF::CapacityManagement::InvalidCapacity("Invalid allocation property", capacities);
            }
            PropertyInterface* property = this->getPropertyFromId(id);
            if(!property){
                throw CF::CapacityManagement::InvalidCapacity("UNKNOWN PROPERTY", capacities);
    ...
    throw (CORBA::SystemException, CF::CapacityManagement::InvalidCapacity, CF::InvalidState) {
        ...
        try{
            for (ii = 0; ii < capacities.length(); ++ii) {
                ...
                if (id == "FRONTEND::tuner_allocation"){
                    ...
                    if (frontend_tuner_allocation.allocation_id.empty()) {
                        ...
                        throw CF::CapacityManagement::InvalidCapacity("MISSING ALLOCATION_ID", capacities);
                    }
                    ...
                    for (size_t tuner_id = 0; tuner_id < tuner_allocation_ids.size(); tuner_id++) {
                        ...
                        if(frontend_tuner_allocation.device_control){
                            ...
                            if(frontend_tuner_allocation.tuner_type == "CHANNELIZER" || frontend_tuner_allocation.tuner_type == "TX"){
                                ...
                                throw CF::CapacityManagement::InvalidCapacity(eout.str().c_str(), capacities);
                            }
                            // listener
                    ...
                    if (frontend_listener_allocation.existing_allocation_id.empty()){
                        ...
                        throw CF::CapacityManagement::InvalidCapacity("MISSING EXISTING ALLOCATION ID", capacities);
                    }
                    if (frontend_listener_allocation.listener_allocation_id.empty()){
                        ...
                        throw CF::CapacityManagement::InvalidCapacity("MISSING LISTENER ALLOCATION ID", capacities);
                    }
                    ...
                    throw CF::CapacityManagement::InvalidCapacity("UNKNOWN ALLOCATION PROPERTY2", capacities);
                }
            }
        }
        catch(const std::logic_error &e) {
            ...
            throw static_cast<CF::CapacityManagement::InvalidCapacity>(e); 
        }
        catch(CF::CapacityManagement::InvalidCapacity &e) {
            try {
                deallocateCapacity(capacities);
            } catch ( ... ) {}
            throw e;
    ...
    */
        ...
        for (CORBA::ULong ii = 0; ii < capacities.length(); ++ii) {
            try{
                ...
                if (id != "FRONTEND::tuner_allocation" && id != "FRONTEND::listener_allocation"){
                    ...
                    throw CF::CapacityManagement::InvalidCapacity("UNKNOWN ALLOCATION PROPERTY", capacities);
                }
                ...
                if (id == "FRONTEND::tuner_allocation"){
                    ...
                    if (tuner_id < 0){
                        ...
                        throw CF::CapacityManagement::InvalidCapacity("ALLOCATION_ID NOT FOUND", capacities);
                    }
                    ...
                    if (this->tuner_allocation_ids[tuner_id].control_allocation_id == frontend_listener_allocation.listener_allocation_id) {
                        ...
                        throw CF::CapacityManagement::InvalidCapacity("Controlling allocation id cannot be used as a listener id", capacities);
                    }
                    // send EOS to listener connection only
    ...
    void FrontendTunerDevice<TunerStatusStructType>::sendEOS(std::string allocation_id) {
        PortServantMap::iterator it;
        for (it = this->_portServants.begin(); it != this->_portServants.end(); it++) {
            if (it->second->getRepid().find("BULKIO") != std::string::npos) {
                ExtendedCF::QueryablePort_ptr prt = ExtendedCF::QueryablePort::_narrow((CORBA::Object_ptr) it->second);
                if (prt->_is_nil()) {
                    continue;
                }
                try {
