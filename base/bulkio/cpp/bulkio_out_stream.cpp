...
public:
    ...
    {
        _modifyingStreamMetadata();
        sca::PropertyMap::cast(_sri->keywords)[name] = value;
        ++_modcount;
    }
    ...
    {
        _modifyingStreamMetadata();
        sca::PropertyMap::cast(_sri->keywords).erase(name);
        ++_modcount;
    }
...
template <class PortType>
void OutputStream<PortType>::setKeyword(const std::string& name, const sca::Value& value)
{
    impl().setKeyword(name, value);
}
...
inline void NumericOutputStream<PortType>::_writeMultiple(const sca::shared_buffer<Sample>& data,
