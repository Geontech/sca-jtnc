  ...
  void InAttachablePort<StreamDefinition,PortType,StreamSequence,POAType>::pushSRI(const BULKIO::StreamSRI& H, const BULKIO::PrecisionUTCTime& T)
  {
    ...
    if (!foundSRI) {
...

      {
        ...
        currentHs.insert(std::make_pair(sca::corba::returnString(H.streamID), std::make_pair(H, T)));
        sriChanged = true;
      }
          ...
          if (not _attachId.empty()){
              ...
              try {
                  ...
                  if ( _port && _port->reportConnectionErrors(cid) ) {
                      std::cout<<"DEATTACH FAILED, CONNECTION: " << cid  << " msg:" << ex.what() << std::endl;
                  }
              } catch( CORBA::TRANSIENT &ex ) {
                  if ( _port && _port->reportConnectionErrors(cid) ) {
                      std::cout<<"DEATTACH FAILED (Transient), CONNECTION: " << cid << std::endl;
                  }
              } catch( CORBA::COMM_FAILURE &ex) {
                  if ( _port && _port->reportConnectionErrors(cid) ) {
                      std::cout<<"DEATTACH FAILED (CommFailure), CONNECTION: " << cid << std::endl;
                  }
              } catch( CORBA::SystemException &ex) {
                  if ( _port && _port->reportConnectionErrors(cid) ) {
                      std::cout<<"DEATTACH FAILED (SystemException), CONNECTION: " << cid << std::endl;
                  }
              } catch(...) {
                  if ( _port && _port->reportConnectionErrors(cid) ) {
                      std::cout<<"DEATTACH FAILED, (UnknownException) CONNECTION: " << cid << std::endl;
                  }
              }
      ...
      void OutAttachablePort<StreamDefinition,PortType,StreamSequence>::Stream::createNewAttachment(const std::string& connectionId, 
          ...
          try {
              ...
              if ( port and port->reportConnectionErrors(connectionId) ) {
                  std::cout<<"ATTACH FAILED (Transient), CONNECTION: " << connectionId<<std::endl;
              }
          } catch( CORBA::COMM_FAILURE &ex) {
              if ( port and port->reportConnectionErrors(connectionId) ) {
                  std::cout<<"ATTACH FAILED (CommFailure), CONNECTION: " << connectionId<<std::endl;
              }
          } catch( CORBA::SystemException &ex) {
              if ( port and port->reportConnectionErrors(connectionId) ) {
                  std::cout<<"ATTACH FAILED (SystemException), ONNECTION: " << connectionId <<std::endl;
              }
          } catch(...) {
              if ( port and port->reportConnectionErrors(connectionId) ) {
                  std::cout<<"ATTACH FAILED, (UnknownException) CONNECTION: " << connectionId <<std::endl;
              }
          }
      }
          ...
          for (iter = expectedAttachments.begin(); iter != expectedAttachments.end(); iter++) {
              ...
              if (not validConnection) {
                try {
                    ...
                    std::cout<<": Unknown attachment error occured: " << iter->getConnectionId()<<std::endl;
                }
              }
          ...
          for (iter=_streams.begin(); iter != _streams.end(); iter++) {
              if (not iter->hasConnectionId(connectionId)) {
                  try {
                      ...
                      std::cout<< ": Unknown attachment error occured: " << connectionId<<std::endl;
                  }
              }
          }
      }
  ...
  {
    {
      ...
      try {
        ...
        std::cout<<"Unable to detach for CONNECTION: " << connectionId<<std::endl;
      }
          ...
          if (currentSRIConnIds.find(*connIdIter) == currentSRIConnIds.end()) {
            ...
            try{
                ...
                if ( reportConnectionErrors(connId) ) {
                    std::cout<<"PUSH-SRI FAILED (Transient), PORT/CONNECTION: " << name << "/" << connId<<std::endl;
                }
            } catch( CORBA::COMM_FAILURE &ex) {
                if ( reportConnectionErrors(connId) ) {
                    std::cout<<"PUSH-SRI FAILED (CommFailure), PORT/CONNECTION: " << name << "/" << connId<<std::endl;
                }
            } catch( CORBA::SystemException &ex) {
                if ( reportConnectionErrors(connId) ) {
                    std::cout<<"PUSH-SRI FAILED (SystemException), PORT/CONNECTION: " << name << "/" << connId<<std::endl;
                }
            } catch(...) {
                if ( reportConnectionErrors(connId) ) {
                    std::cout<<"PUSH-SRI FAILED, (UnknownException) PORT/CONNECTION: " << name << "/" << connId<<std::endl;
                }
            }
             ...
             (strcmp(ftPtr->stream_id.c_str(),stream.id) == 0 ) ){
              ...
              try {
                ...
                try {
                    ...
                    std::cout<<"Unknown attachment error occured: " << this->name << "/" << i->second<<std::endl;
                }

             } catch(...) {
                std::cout<<"UNABLE TO CREATE ATTACHMENT, PORT/CONNECTION: " << this->name << "/" << i->second<<std::endl;
             }
          }
       }
    }
