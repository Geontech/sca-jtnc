...
namespace bulkio {
    ...
    protected:
        ...
        void _receivePushPacket(MessageBuffer& msg)
        {
            ...
            if (count > 0) {
                ...
                if (inband_data) {
                    sca::buffer<NativeType> temp(count);
        ...
        void _receiveSharedBuffer(MessageBuffer& msg, BufferType& buffer, size_t size)
        {
            ...
            if ((start * sizeof(NativeType)) != offset) {
                ...
                sca::shared_buffer<char> temp(ptr, bytes, &sca::shm::HeapClient::deallocate, sca::detail::process_shared_tag());
                temp.trim(offset);
                ...
                size += start;
                buffer = BufferType(ptr, size, &sca::shm::HeapClient::deallocate, sca::detail::process_shared_tag());
                if (start > 0) {
                    buffer.trim(start);
                }
            }
        ...
        FifoEndpoint _fifo;
        sca::shm::HeapClient _heapClient;
    };
    ...
    CF::Properties ShmInputManager<PortType>::transportProperties()
    {
        ...
        sca::corba::push_back(properties, sca::PropertyType("hostname", std::string(host)));
        return properties;
    }
                                                                              ...
                                                                              const sca::PropertyMap& properties)
    {
        ...
        try {
            ...
            throw sca::FatalTransportError("failed to connect to FIFO " + location);
        }
    }

    template <class PortType>
    sca::PropertyMap ShmInputManager<PortType>::getNegotiationProperties(sca::ProvidesTransport* providesTransport)
    {
