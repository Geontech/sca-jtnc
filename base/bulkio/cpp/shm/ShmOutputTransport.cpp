...
namespace bulkio {
    ...
    protected:
        virtual void _pushSRI(const BULKIO::StreamSRI& sri)
        {
            try {
                ...
                throw sca::FatalTransportError(sca::corba::describeException(exc));
            }
        }
                                 ...
                                 const std::string& streamID)
        {
            ...
            if (!data.empty()) {
                ...
                if (data.get_memory().is_process_shared()) {
                    ...
                    ElementType* ptr = static_cast<ElementType*>(sca::shm::allocate(bytes));
                    if (ptr) {
                        ...
                        copy = BufferType(ptr, count, sca::shm::deallocate);
        ...
        virtual sca::PropertyMap _getExtendedStatistics()
        {
    ...
    private:
        ...
        {
            sca::shm::MemoryRef ref = sca::shm::Heap::getRef(base);
            if (!ref) {
        ...
        void _sendMessage(const void* header, size_t hsize, const void* body, size_t bsize)
        {
            try {
                ...
                throw sca::FatalTransportError(exc.what());
            }

            size_t status;
            if (_fifo.read(&status, sizeof(size_t)) != sizeof(size_t)) {
                throw sca::FatalTransportError("failed to read response");
            } else if (status != 0) {
                throw sca::TransportError("call failed");
            }
        }
    ...
    CF::Properties ShmOutputManager<PortType>::transportProperties()
    {
        ...
        sca::corba::push_back(properties, sca::PropertyType("hostname", _hostname));
        return properties;
    }
    ...
    template <typename PortType>
    sca::PropertyMap ShmOutputManager<PortType>::getNegotiationProperties(sca::UsesTransport* transport)
    {
        ...
        sca::PropertyMap properties;
        properties["fifo"] = shm_transport->getFifoName();
        return properties;
    }

    template <typename PortType>
    void ShmOutputManager<PortType>::setNegotiationResult(sca::UsesTransport* transport,
                                                          const sca::PropertyMap& properties)
    {
        TransportType* shm_transport = dynamic_cast<TransportType*>(transport);
        if (!shm_transport) {
        ...
        if (!properties.contains("fifo")) {
            throw sca::FatalTransportError("invalid properties for shared memory connection");
        }
