...
namespace bulkio {
    ...
    void OutputTransport<PortType>::disconnect()
    {
        ...
        for (VersionMap::iterator stream = _sriVersions.begin(); stream != _sriVersions.end(); ++stream) {
            ...
            } catch (sca::TransportTimeoutError& e) {
    ...
    BULKIO::PortStatistics OutputTransport<PortType>::getStatistics()
    {
        ...
        for (VersionMap::iterator stream = _sriVersions.begin(); stream != _sriVersions.end(); ++stream) {
            sca::corba::push_back(statistics.streamIDs, stream->first.c_str());
        }

        // Add extended statistics from subclasses to the keywords
        sca::corba::extend(statistics.keywords, _getExtendedStatistics());
    ...
    template <typename PortType>
    sca::PropertyMap OutputTransport<PortType>::_getExtendedStatistics()
    {
        return sca::PropertyMap();
    }
    ...
    InputTransport<PortType>::InputTransport(InPortType* port, const std::string& transportId) :
        sca::ProvidesTransport(port, transportId),
        _port(port)
    {
    }
    ...
    template <typename PortType>
    sca::ProvidesTransportManager*
    BulkioTransportFactory<PortType>::createProvidesManager(sca::NegotiableProvidesPortBase* port)
    {
    ...
    template <typename PortType>
    sca::UsesTransportManager*
    BulkioTransportFactory<PortType>::createUsesManager(sca::NegotiableUsesPort* port)
    {
