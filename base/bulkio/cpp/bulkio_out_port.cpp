...
namespace bulkio {
  ...
  */
  ...
  void OutPort<PortType>::pushSRI(const BULKIO::StreamSRI& H)
  {
      ...
      if (active) {
          for (TransportIterator connection = _connections.begin(); connection != _connections.end(); ++connection) {
              ...
              } catch (const sca::FatalTransportError& err) {
                  std::cout<<"PUSH-SRI FAILED " << err.what()
                            << " PORT/CONNECTION: " << name << "/" << connection_id<<std::endl;
              }
          }
      }
  }
    ...
    if (active) {
        for (TransportIterator connection = _connections.begin(); connection != _connections.end(); ++connection) {
            ...
            try {
                ...
                std::cout<<"PUSH-PACKET FAILED " << err.what()
                          << " PORT/CONNECTION: " << name << "/" << connection_id<<std::endl;
                transport->setAlive(false);
            } catch (const sca::TransportError& err) {
                std::cout<<"pushPacket error on connection '" << connection_id << "': " << err.what()<<std::endl;
            }
        }
    }
  ...
  BULKIO::UsesPortStatisticsSequence* OutPort<PortType>::statistics()
  {
      ...
      for (TransportIterator connection = _connections.begin(); connection != _connections.end(); ++connection) {
          ...
          sca::corba::push_back(recStat, stat);
      }
  ...
  OutPort<PortType>::_createTransport(CORBA::Object_ptr object, const std::string& connectionId)
  {
      ...
      try {
          ...
          std::cout<<"CONNECT FAILED: UNABLE TO NARROW ENDPOINT,  USES PORT:" << name<<std::endl;
          throw CF::Port::InvalidPort(1, "Unable to narrow");
      }
  ...
  typename OutPort<PortType>::ConnectionsList  OutPort<PortType>::getConnections()
  {
    ...
    for (ConnectionList::iterator iter = _connections.begin(); iter != _connections.end(); ++iter) {
        PortVarType port = sca::corba::_narrowSafe<PortType>((*iter)->objref);
  ...
  */
  ...
  OutNumericPort<PortType>::OutNumericPort(const std::string& name,
                                               ConnectionEventListener *connectCB,
                                               ConnectionEventListener *disconnectCB) :
    OutPort<PortType>(name, connectCB, disconnectCB)
  {
  }
  ...
  OutBitPort::OutBitPort(const std::string& name) :
    OutPort<BULKIO::dataBit>(name)
  {
  }


  void OutBitPort::pushPacket(const sca::shared_bitbuffer& data, const BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID)
  {
  ...
  OutFilePort::OutFilePort(const std::string& name,
                           ConnectionEventListener *connectCB,
                           ConnectionEventListener *disconnectCB) :
    OutPort<BULKIO::dataFile>(name, connectCB, disconnectCB)
  {
  }
  ...
  OutXMLPort::OutXMLPort(const std::string& name,
                         ConnectionEventListener *connectCB,
                         ConnectionEventListener *disconnectCB) :
    OutPort<BULKIO::dataXML>(name, connectCB, disconnectCB)
  {
  }
