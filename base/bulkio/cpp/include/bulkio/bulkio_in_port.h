...
#include <sca/callback.h>
#include <sca/signalling.h>
...
namespace bulkio {
  ...
  //
  template <typename PortType>
  class InPort : public sca::NegotiableProvidesPortBase, public virtual CorbaTraits<PortType>::POATypeExt
  {
  public:
  ...
  protected:
    ...
    //
    sca::callback<void (BULKIO::StreamSRI&)> newStreamCallback;
    ...
    //
    sca::signal<std::string> packetWaiters;

    //
    // Notification for new stream creation
    //
    sca::notification<void (StreamType)> streamAdded;
  ...
  {
  public:
    InBitPort(const std::string& name);
