...
namespace bulkio {
  ...
  class OutPort : public sca::NegotiableUsesPort, public virtual POA_BULKIO::internal::UsesPortStatisticsProviderExt
  {
  ...
  protected:
    ...
    virtual sca::UsesTransport* _createLocalTransport(PortBase* port, CORBA::Object_ptr object, const std::string& connectionId);

    virtual sca::UsesTransport* _createTransport(CORBA::Object_ptr object, const std::string& connectionId);

    typedef sca::UsesPort::TransportIteratorAdapter<PortTransportType> TransportIterator;
  ...
  public:
    ...
    OutBitPort(const std::string& name); 

    void pushPacket(const sca::shared_bitbuffer& data, const BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID);
  };
