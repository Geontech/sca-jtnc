...
SampleDataBlock<T>::SampleDataBlock(const BULKIO::StreamSRI& sri, size_t size) :
    Base(bulkio::StreamDescriptor(sri), sca::buffer<T>(size))
{
}
...
#define INSTANTIATE_NUMERIC_TEMPLATE(x)                 \
    INSTANTIATE_TEMPLATE(sca::shared_buffer<x>);    \
...
INSTANTIATE_TEMPLATE(sca::shared_bitbuffer);
