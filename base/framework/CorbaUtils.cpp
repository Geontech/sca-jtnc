...
static CORBA::Boolean handleCommFailure (void* cookie, CORBA::ULong retry, const CORBA::COMM_FAILURE& ex)
{
    ...
    if (maxRetries < 0) {
        if (retry == 0) {
            std::cout<<"CORBA::COMM_FAILURE retrying indefinitely"<<std::endl;
        }
        return true;
    } else if ((int)retry < maxRetries) {
        std::cout<<"CORBA::COMM_FAILURE retrying " << retry<<std::endl;
        return true;
    } else {
        std::cout<<"CORBA::COMM_FAILURE not retrying " << retry<<std::endl;
        return false;
    }
}
...
std::string describeException(const CORBA::Exception& exc) {
  ...
  CosNaming::NamingContext_ptr CreateNamingContextPath(const std::string &namingContext) {
	...
	std::cout<<"CreateNamingContextPath"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
      }
    }
    return ctx;
  }
  ...
  CosNaming::NamingContext_ptr ResolveNamingContextPath( const std::string &namingContext ) {
	...
	std::cout<<"ResolveNamingContextPath"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
      }
    }
    return ctx;
  }
  ...
  int  DeleteNamingContext( const std::string &namingContext )  {
	...
	std::cout<<"DeleteNamingContext"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
      }
     }
    return retval;
  }
  ...
  int  DeleteNamingContextPath( const std::string &namingContext )  {
	...
	for ( ; r != nc_list.rend(); ++r ) {
	  try {
	    ...
	    std::cout<<"DeleteNamingContextPath"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
	    retval=1;
	    break;
	  }
	}
  ...
  int  Unbind( const std::string &name , CosNaming::NamingContext_ptr namingContext ) {
	...
	retval=0;
      ...
      std::cout<<"Unbind"<<" NameContext : CannotProceed "<<std::endl;
    }
    catch(CosNaming::NamingContext::InvalidName &ex) {
      std::cout<<"Unbind"<<" NameContext : InvalidName "<<std::endl;
    }
    catch (const CORBA::Exception& ex) {
      std::cout<<"Unbind"<<" CORBA " << ex._name() << " exception during unbind operation, name:" << name <<std::endl;
    }
  ...
  int  Unbind( const std::string &name , const std::string &namingContext ) {
	...
	std::cout<<"Unbind"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
      }
  ...
  int  Bind(const std::string &name,  CORBA::Object_ptr obj,  CosNaming::NamingContext_ptr namingContext  ) {
	...
	try{
      ...
      std::cout<<"Bind"<<" CORBA " << ex._name() << " exception during bind operation, name:" << name<<std::endl;
    }
  ...
  int  Bind( const std::string &name,  CORBA::Object_ptr obj, const std::string &namingContext, bool create_nc ) {
	...
	std::cout<<"Bind"<<" CORBA " << ex._name() << " exception during, bind context:" << namingContext<<std::endl;
      }
