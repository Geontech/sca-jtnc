...
bool PropertyMap::operator==( const sca::PropertyMap &other ) const
{
    //
    ...
    for ( const_iterator iter = begin(); iter != end(); ++iter) {
        ...
        if (  !sca::compare_anys(iter->getValue(), other_prop->getValue(), action)  ) {
            return false;
        }
    }
...
{
    sca::corba::push_back(*this, property);
}

void PropertyMap::extend(const CF::Properties& properties)
{
    sca::corba::extend(*this, properties);
}
...
std::ostream& sca::operator<<(std::ostream& out, const sca::PropertyMap& properties)
{
    out << "{";
    bool first = true;
