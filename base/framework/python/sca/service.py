...
from sca.resource import load_logging_config_uri
...
def patchService(target):
    ...
    def saveLoggingContext(self, logcfg_url, oldstyle_loglevel, rscCtx ):
        ...
        if oldstyle_loglevel != None and oldstyle_loglevel > -1 :
            log4level = sca.logger.ConvertToLog4Level( sca.logger.ConvertLogLevel(oldstyle_loglevel))
            ...
            if self._log and self._log.getEffectiveLevel() == logging.NOTSET:
                ...
                self._logLevel = sca.logger.ConvertLog4ToCFLevel( logging.getLogger(None).getEffectiveLevel() )
        ...
        try:
            ...
            try:
              if name != "": category=name.rsplit("_", 1)[0]
                ...
                pass 
            ctx = sca.logger.ServiceCtx( name, dpath )
            sca.logger.Configure( log_config_uri, debug_level, ctx, category )
