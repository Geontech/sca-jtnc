...
try:
    ...
    from sca.cf import StandardEvent
except:
    hasEvents = False

import sca.resource as resource
import sca.utils
import sca.logger
from  sca.events import Publisher
from  sca.events import  Manager
import sys
import logging
import signal
import os
...
class Device(resource.Resource):
    ...
    def __init__(self, devmgr, identifier, label, softwareProfile, compositeDevice, execparams, propertydefs=(),loggerName=None):
        ...
        self._deviceLog = self._baseLog.getChildLogger('Device', sca.utils.log4py.SYSTEM_LOGS)
        self._loadableDeviceLog = self._baseLog.getChildLogger('Device', sca.utils.log4py.SYSTEM_LOGS)
        self._executableDeviceLog = self._baseLog.getChildLogger('Device', sca.utils.log4py.SYSTEM_LOGS)
            ...
            if idm_ior != None and idm_ior != "":
                ...
                try:
                    ...
                    if self._domMgr:
                        ...
                        self._idm_publisher = evt_mgr.Publisher( sca.events.IDM_Channel_Spec )
...
class ExecutableDevice(LoadableDevice):
    ...
    def _execute(self, command, options, parameters):
        ...
        # SR:445
        try:
            sp = sca.utils.Popen(args, executable=command, cwd=os.getcwd(), close_fds=True, stdin=self._devnull, preexec_fn=os.setpgrp)
        except OSError, e:
            # SR:455
        ...
        try:
            ...
            try:
              if not category and label != "": category=label.rsplit("_", 1)[0]
                ...
                pass 
            ctx = sca.logger.DeviceCtx( label, id, dpath )
