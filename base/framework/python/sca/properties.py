...
def prf_to_classes(prfFile):
    ...
    """
    props = sca.parsers.prf.parse(prfFile)
    classes = {}
...
def xml_to_class(prop):
    """
    Given an instance of sca.parsers.prf.struct, return a class object that
...
class _property(object):
    ...
    def checkValue(self, value, obj):
        ...
        if type(self) == sca.properties.simple_property:
        ...
        elif type(self) == sca.properties.simpleseq_property:
...
class simple_property(_property):
    ...
    def toXML(self, level=0, version="2.2.2"):
        ...
        simp = sca.parsers.prf.simple(id_=self.id_, 
                                        ...
                                        value=value,
                                        units=self.units,
                                        action=sca.parsers.prf.action(type_=self.action))

        for kind in self.kinds:
            simp.add_kind(sca.parsers.prf.kind(kindtype=kind))
