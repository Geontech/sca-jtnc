...
import sca.utils.log4py.config
import tempfile
import os
import urlparse
...
from sca.properties import PropertyStorage
from sca.utils import uuid
from sca.threadedcomponent import ThreadedComponent
import sca.logger
import containers
...
class _EC_PropertyChangeListener(object):
    def __init__(self, ec ):
        ...
        try:
            ...
            if channel:
                self.ec = channel
                self.pub = sca.events.Publisher(self.ec)
            else:
...
class Resource(object):
    def __init__(self, identifier, execparams, propertydefs=(), loggerName=None, baseLogName=None):
        ...
        self._logLevel = None
        self.logConfig = ""
        self.loggingMacros = sca.logger.GetDefaultMacros()
        sca.logger.ResolveHostInfo( self.loggingMacros )
        ...
        self._resourceLog = self._baseLog.getChildLogger('Resource', sca.utils.log4py.SYSTEM_LOGS)
        self._portSupplierLog = self._baseLog.getChildLogger('PortSupplier', sca.utils.log4py.SYSTEM_LOGS)
        self._propertySetLog = self._baseLog.getChildLogger('PropertySet', sca.utils.log4py.SYSTEM_LOGS)
        loglevel = self._log.getEffectiveLevel()
        if loglevel == logging.NOTSET:
            self.logLevel = logging.INFO
        else:
            self.logLevel = loglevel
        self._logLevel = sca.logger.ConvertLog4ToCFLevel( self.logLevel )
    ...
    def setAdditionalParameters(self, softwareProfile, application_registrar_ior, nic):
        ...
        try:
            ...
            if applicationRegistrar != None:
                ...
                self._ecm = sca.events.Manager.GetManager(self)
        except:
            self._domMgr = None
    ...
    def releaseObject(self):
        ...
        sca.logger.SetEventChannelManager(None)
        # release all event channels
        if self._ecm: sca.events.Manager.Terminate()
    ...
    def setLoggingContext(self, logcfg_url, oldstyle_loglevel, rscCtx ):
        ...
        if oldstyle_loglevel != None and oldstyle_loglevel > -1 :
            self.setLogLevel( "", sca.logger.ConvertLogLevel(oldstyle_loglevel) )
    ...
    def saveLoggingContext(self, logcfg_url, oldstyle_loglevel, rscCtx ):
        ...
        if logcfg_url==None or logcfg_url=="" :
            self.logConfig = sca.logger.GetDefaultConfig()
        else:
            # try to process URL and grab contents
            try:
                cfg_data=sca.logger.GetConfigFileContents( logcfg_url )
                if cfg_data and len(cfg_data) > 0 :
                    self.logConfig = sca.logger.ExpandMacros( cfg_data, self.loggingMacros )
                    self.logConfig = sca.logger.ConfigureWithContext( self.logConfig, self.loggingMacros  )
            except:
                pass

        # apply logging level if explicitly stated
        if oldstyle_loglevel != None and oldstyle_loglevel > -1 :
            loglevel = sca.logger.ConvertLogLevel(oldstyle_loglevel)
            self.setLogLevel( "", loglevel )
            if self._log and self._log.getEffectiveLevel() == logging.NOTSET:
                self.setLevel( self._logid, loglevel )
        else:
            if self._log and self._log.getEffectiveLevel() == logging.NOTSET:
                loglevel = sca.logger.ConvertLog4ToCFLevel( logging.getLogger(None).getEffectiveLevel() )
                self.setLogLevel( self._logid, loglevel )

        # assign an event channel manager to the logging library
        sca.logger.SetEventChannelManager( self._ecm )
    ...
    def _get_log_level(self):
        if self._baseLog:
            lvl = sca.logger.ConvertLog4ToCFLevel( self._baseLog.getEffectiveLevel())
    ...
    def log_level(self, newLogLevel=None ):
        ...
        if sca.logger.SupportedCFLevel(newLogLevel) == False:
           return
        ...
        if self.logListenerCallback and callable(self.logListenerCallback.logLevelChanged):
            ...
            sca.logger.SetLogLevel( self._logid, newLogLevel )
            self._logLevel = newLogLevel
            self.logLevel = sca.logger.ConvertToLog4Level( newLogLevel )
    ...
    def setLogLevel(self, logid, newLogLevel ):
        ...
        if self.logListenerCallback and callable(self.logListenerCallback.logLevelChanged):
            ...
            sca.logger.SetLogLevel( logid, newLogLevel )
    ...
    def getLogLevel(self, logid ):
        ...
        lvl = sca.logger.ConvertLog4ToCFLevel(logging.getLogger(logid).getEffectiveLevel())
        return lvl
    ...
    def resetLog(self):
        for _logid in self._baseLog.getCurrentLoggers():
            if _logid == self._baseLog.name:
                sca.logger.SetLogLevel(_logid, -1)
            _b_id = _logid
            while _b_id != self._baseLog.name:
                sca.logger.SetLogLevel(_b_id, -1)
    ...
    def setLogConfig(self, new_log_config):
        if self.logListenerCallback and callable(self.logListenerCallback.logConfigChanged):
            tcfg= sca.logger.ExpandMacros( new_log_config, self.loggingMacros  )
            if tcfg:
               self.logConfig = tcfg
               self.logListenerCallback.logConfigChanged(tcfg)
        elif new_log_config:
            tcfg= sca.logger.ConfigureWithContext( new_log_config, self.loggingMacros  )
            if tcfg:
                self.logConfig = tcfg
    ...
    def setLogConfigURL(self, log_config_url):
        if log_config_url:
            fc=sca.logger.GetConfigFileContents( log_config_url )
            ...
            try:
                ...
                for propid in self._props.keys():
                    if self._props.has_id(propid) and self._props.isQueryable(propid):
                        ...
                        if type(prp) == sca.properties.struct_property:
                            newvalval = []
...
def load_logging_config_uri(orb, uri, binding=None):
    ...
    if scheme == "file":
        sca.utils.log4py.config.fileConfig(path, binding)
    elif scheme == "sca":
        ...
        try:
            ...
            if fileSys == None:
                ...
                try:
                    ...
                    sca.utils.log4py.config.fileConfig(t)
                finally:
                    os.remove(t)
    else:
        # Invalid scheme
...
def start_component(componentclass, interactive_callback=None, thread_policy=None, loggerName=None):   
    ...
    try:
        try:
            ...
            ctx = sca.logger.ComponentCtx(
                name = name_binding,
                id = component_identifier,
                dpath = dpath )
            sca.logger.Configure(
