...
/*{% endif %}*/
    ...
    Ports:
        ...
        More advanced uses of input streams are possible; refer to the SCA documentation
        for more details.
        ...
        Example:
            ...
            sca::shared_buffer<short> inputData = block.buffer();

            // Acquire a new buffer to hold the output data
            sca::buffer<float> outputData(inputData.size());
        ...
        buffer without making a copy:
            ...
            if (block.complex()) {
                sca::shared_buffer<std::complex<short> > inData = block.cxbuffer();
                sca::buffer<std::complex<float> > outData(inData.size());
