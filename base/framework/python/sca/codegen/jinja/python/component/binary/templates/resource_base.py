...
from sca.utils import uuid, model

#{% if superclass == "ExecutableDevice" %}
from sca.device import LoadableDevice
#{% endif %}
...
from sca.properties import simpleseq_property
#{% endif %}
...
#{%   if loop.first %}
from sca.resource import usesport, providesport, PortCallError
#{%   endif %}
...
class ${className}(${component.poaclass}, ${component.superclasses|join(', ', attribute='name')}):
        ...
        def __init__(self, identifier, execparams):
            ...
            self.scaroot = os.getenv('SCAROOT')
...
#{% endfor %}
        ...
        def run_binary(self):
            ...
            if status: # in the sandbox, file paths are absolute, not based on $SCAROOT
