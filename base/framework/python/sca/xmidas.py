...
warnings.warn("sca.xmidas is deprecated, use xmsca.resource instead", DeprecationWarning)
...
try:
    ...
    # If it's not available, provide the last version of sca.xmidas
    # This code should eventually be removed.
    from sca.cf import CF
    from sca.properties import simple_property, struct_property
    ...
    class ResourceControlPanel(ControlPanel):
        """Provides a pre-populated control panel for sca.resource.Resource objects.

        When possible, this control panel will be pre-populated with widgets that
        are tied to all properties defined on the component (using sca.properties).
        """
...
if __name__ == "__main__":
    from sca.resource import Resource
    from sca.cf import CF__POA
    import logging
