...
BasicFEI_base::BasicFEI_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl) :
...
BasicFEI_base::BasicFEI_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, char *compDev) :
...
BasicFEI_base::BasicFEI_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities) :
...
BasicFEI_base::BasicFEI_base(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev) :
...
void BasicFEI_base::construct()
{
    ...
    this->addPropertyListener(connectionTable, this, &BasicFEI_base::connectionTableChanged);
...
*******************************************************************************************/
void BasicFEI_base::start() throw (CORBA::SystemException, CF::ControllableInterface::StartError)
{
    frontend::FrontendTunerDevice<frontend_tuner_status_struct_struct>::start();
    ThreadedComponent::startThread();
}

void BasicFEI_base::stop() throw (CORBA::SystemException, CF::ControllableInterface::StopError)
{
...
void BasicFEI_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void BasicFEI_base::connectionTableChanged(const std::vector<connection_descriptor_struct>* oldValue, const std::vector<connection_descriptor_struct>* newValue)
{
    dataShort_out->updateConnectionFilter(*newValue);
}

void BasicFEI_base::loadProperties()
{
...
void BasicFEI_base::frontendTunerStatusChanged(const std::vector<frontend_tuner_status_struct_struct>* oldValue, const std::vector<frontend_tuner_status_struct_struct>* newValue)
{
    this->tuner_allocation_ids.resize(this->frontend_tuner_status.size());
}

CF::Properties* BasicFEI_base::getTunerStatus(const std::string& allocation_id)
{
...
void BasicFEI_base::assignListener(const std::string& listen_alloc_id, const std::string& allocation_id)
{
...
void BasicFEI_base::removeListener(const std::string& listen_alloc_id)
{
...
void BasicFEI_base::removeAllocationIdRouting(const size_t tuner_id) {
...
void BasicFEI_base::removeStreamIdRouting(const std::string stream_id, const std::string allocation_id) {
    std::vector<connection_descriptor_struct> old_table = this->connectionTable;
    std::vector<connection_descriptor_struct>::iterator itr = this->connectionTable.begin();
    while (itr != this->connectionTable.end()) {
        if (allocation_id == "") {
...
void BasicFEI_base::matchAllocationIdToStreamId(const std::string allocation_id, const std::string stream_id, const std::string port_name) {
    if (port_name != "") {
